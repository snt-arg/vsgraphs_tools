...
def main():
    ...
    parser.add_argument(
        "--mode",
        choices=["voxblox_receive"],
        required=True,
        help="Run as Voxblox Skeleton Reciever (client)!",
    )
    parser.add_argument(
        "--host",
        default="127.0.0.1",
    ...
    # Initialize ROS 2
    rclpy.init()
    if args.mode == "voxblox_receive":
        node = JazzyRelay(host=args.host)
    else:
        raise ValueError("Invalid mode! Use 'voxblox_receive'.")
    # Spin the node
    try:
        rclpy.spin(node)
